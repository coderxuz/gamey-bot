...
async def games_response(translate: LangType, page: int, total_pages:int, completed: bool, game_id_list: List[list[int]]):
    ...
    if page > 0 and page!=1:
        prev_btn = InlineKeyboardButton(text="previous", callback_data=f"page:{completed}:{page-1}")
        keyboard.row(prev_btn)
    if page<total_pages-1:
        next_btn= InlineKeyboardButton(text="Next", callback_data=f"page:{completed}:{page+1}")
